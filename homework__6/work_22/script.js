...
const sortById = (posts) => {
  const sortByIdPosts = posts.sort((a,b) => a.id < b.id ? -1 : 1);
  return sortByIdPosts;
};
// SORTING FUNCTION BY LENGTH (post.body.length)
const sortByCommentLength = (posts) => {
  const sortByCommentLengthPosts = posts.sort((a,b) => a.body.length < b.body.length ? -1 : 1);
  return sortByCommentLengthPosts;
};